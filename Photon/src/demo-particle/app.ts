...
class ParticleDemo extends Photon.LoadBalancing.LoadBalancingClient {
    ...
    public useGroups: bool = false;
    public automove: bool = false;

    // connect to random room or create new one automatocally 
    // close button click sets this to false
    public autoconnect: bool = true;

    constructor(private canvas: HTMLCanvasElement) {
        super(ParticleDemoMasterAddress, ParticleDemoAppId, ParticleDemoAppVersion);
    ...
    private logger = new Exitgames.Common.Logger("Demo: ", DemoConstants.LogLevel);

    // overrides
    private roomFactory(name: string) { return new ParticleRoom(name); }
    private actorFactory(name: string, actorNr: number, isLocal: bool) { return new ParticlePlayer(this, name, actorNr, isLocal); }
...
//            }
    ...
    // ui
    private setupUI() {
        this.canvas.onclick = (mouseEvent) => {
            var relativeClickX = mouseEvent.clientX - this.canvas.offsetLeft;
            var relativeClickY = mouseEvent.clientY - this.canvas.offsetTop;
            ...
            this.myActor().moveLocal(Math.floor(relativeClickX / this.cellWidth), Math.floor(relativeClickY / this.cellHeight));
            this.stage.update();
        }
...
class Output {
    static public logger = new Exitgames.Common.Logger();

    static log(str: string, ...op: any[]) {
        var log = document.getElementById("log");
        var formatted = logger.formatArr(str, op);
...
class ParticlePlayer extends Photon.LoadBalancing.Actor {
    constructor (private game: ParticleDemo, name: string, actorNr: number, isLocal: bool) {

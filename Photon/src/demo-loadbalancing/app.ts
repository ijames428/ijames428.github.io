...
class DemoLoadBalancing extends Photon.LoadBalancing.LoadBalancingClient {
    private logger = new Exitgames.Common.Logger("Demo: ");
    ...
    public onFbToken = function (token) {
        this.setCustomAuthentication("token=" + token);
        this.output("Got fb token. Setting custom fb authentication.");
        this.connect(true);
    };
}
